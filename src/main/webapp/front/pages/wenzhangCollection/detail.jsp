...
<body>
        ...
        <section class="blog-details-section pt-120 pb-120">
            <div class="data-detail-breadcrumb" style="width: 1110px; padding: 0px 10px; box-shadow: rgba(255, 0, 0, 0.3) 0px 0px 0px; margin: 20px auto; border-color: rgb(135, 206, 250); border-radius: 4px; border-width: 0px; border-style: solid; height: 54px;">
                <span class="layui-breadcrumb">
                    ...
                    <a><cite>{{detail.wenzhangCollectionName}}</cite></a>
                </span>
    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-8">
                <div class="blog-details-wrapper">
                    <div><img :src="detail.wenzhangCollectionPhoto" alt="image" style="width: 80%;height: auto" ></div>
                    <div class="blog-details-header">
                        <h3 class="blog-details-title">{{title}}</h3>
                        <ul class="post-meta">
                            <li v-if="detail.wenzhangCollectionTypes">类型：
                                {{detail.wenzhangCollectionValue}}
                            </li>
                        </ul>
                        ...
                        <!--<div style="padding: 20px;border: 0px solid #f2f2f2;margin-top: 20px;display: flex">
                            <div class="num-picker">
                                <button @click="wenzhangCollectionYuyue()" style="height:auto;" :style='{"padding":"0 10px","boxShadow":"0 0 0px rgba(255,0,0,.3)","margin":"0 5px","borderColor":"rgba(0,0,0,.3)","backgroundColor":"rgba(23, 124, 176, 1)","color":"rgba(255, 255, 255, 1)","borderRadius":"6px","borderWidth":"0","width":"auto","lineHeight":"40px","fontSize":"16px","borderStyle":"solid"}' type="button" class="layui-btn btn-submit">
                                    立即预约
                                </button>
                            </div>
                        </div>-->
        ...
        <div class="col-lg-4" v-if="wenzhangCollectionRecommendList">
            <div class="sidebar">
                ...
                <div class="widget">
                    ...
                    <ul class="small-post-list">
                        <li class="small-post-single" v-for="(item,index) in wenzhangCollectionRecommendList" v-bind:key="index" @click="jump('../wenzhangCollection/detail.jsp?id='+item.id)">
                            <div class="thumb"><img width="65px" :src="item.wenzhangCollectionPhoto"  alt="image"></div>
                            <div class="content">
                                <a class="main_color" class="date">{{item.wenzhangCollectionName}}</a>
                                <h6 v-if="item.wenzhangCollectionNewMoney" class="post-title"><a style="color: red">{{item.wenzhangCollectionNewMoney}}</a>RMB</h6>
                            </div>
                        </li>
                    </ul>
                </div>
...
<script>
    ...
    var vue = new Vue({
        ...
        data: {
            ...
            //系统推荐
            wenzhangCollectionRecommendList: [],
            dataList: [],
            // 当前详情页表
            detailTable: 'wenzhangCollection',
        },
        //  清除定时器
        destroyed: function () {
            // 不知道具体作用
        ...
        methods: {
            ...
            //预约
            ,wenzhangCollectionYuyue(){
                let _this = this;
                // localStorage.setItem('wenzhangCollectionId', _this.detail.id);
                // _this.jump("../wenzhangCollectionOrder/add.jsp");

                let data={
                    wenzhangCollectionId:_this.detail.id,
                    yonghuId:localStorage.getItem("userid"),
                    wenzhangCollectionOrderYesnoTypes:1,
                }
                // 提交数据
                layui.http.requestJson('wenzhangCollectionOrder/add', 'post', data, function (res) {
                    if(res.code == 0) {
                        layui.layer.msg('预约成功', {
                            time: 2000,
                            icon: 6
                        }, function () {
                            _this.jump("../wenzhangCollectionOrder/list.jsp");
                        });
                    }else{
    ...
    layui.use(['layer', 'form', 'element', 'carousel', 'http', 'jquery', 'laypage', 'rate'], function () {
        ...
        http.request(`${vue.detailTable}/detail/` + id, 'get', {}, function (res) {
            // 详情信息
            vue.detail = res.data;
            vue.title = vue.detail.wenzhangCollectionName;
           // 轮播图片
            vue.swiperList = vue.detail.wenzhangCollectionPhoto ? vue.detail.wenzhangCollectionPhoto.split(",") : [];
            var swiperItemHtml = '';
        ...
        http.request(`wenzhangCollection/list`, 'get', {
            page: 1,
            limit: 5,
            wenzhangCollectionTypes: vue.detail.wenzhangCollectionTypes,




        }, function (res) {
            vue.wenzhangCollectionRecommendList = res.data.list;
        });
