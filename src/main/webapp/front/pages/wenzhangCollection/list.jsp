            ...
            <div class="ng-inmod-ini">
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="wenzhangUuidNumber" id="wenzhangUuidNumber" placeholder="文章编号" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="wenzhangName" id="wenzhangName" placeholder="文章名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="wenzhangTypes" id="wenzhangTypes" lay-filter="wenzhangTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in wenzhangTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                ...
                <div class="ng-inside-box ng-fault ng-container" aos="fade-up">
                    ...
                    <div class="travel-gallery-wrapper">
                        <div class="row">
                            <div v-for="(item,index) in dataList" v-bind:key="index" class="col-md-3 col-sm-6 col-xs-6 col-lg-3">
                                <i @click="deleteWenzhangCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: 0px;margin-right: 3px;font-size:20px">&#xe640;</i>
                                <div class="single-gallery-item" @click="jumpCheck('../wenzhang/detail.jsp?id='+item.wenzhangId , item.wenzhangDelete == null?'':item.wenzhangDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)">
                                    <div class="img-box">
                                        <img :src="item.wenzhangPhoto?item.wenzhangPhoto.split(',')[0]:''" style="height: 350px;width:100%;" />
                                        <b class="location" v-if="item.wenzhangNewMoney">
                                            {{item.wenzhangNewMoney}} RMB
                                        </b>
                                        <b class="location" v-else>
                                            {{item.wenzhangValue}}
                                        </b>
                                    </div>
                                    <div class="content">
                                        <h5 class="catagory">
                                            <b style="color: inherit">{{item.wenzhangName}}</b>
                                        </h5>
                                    </div>
                                </div>
                            </div>
...
<script>
    var vue = new Vue({
        ...
        methods: {
            ...
            ,deleteWenzhangCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('wenzhangCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    // 当前表的字典表
        ...
        function wenzhangCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=wenzhang_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.wenzhangCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function wenzhangTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=wenzhang_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.wenzhangTypesList = res.data.list;
                }
            });
        }
        ...
        function pageList() {
            ...
            if (jquery('#wenzhangUuidNumber').val() != null && jquery('#wenzhangUuidNumber').val() != "" && jquery('#wenzhangUuidNumber').val() != "null" ) {
                param['wenzhangUuidNumber'] = jquery('#wenzhangUuidNumber').val();
            }
                     
            if (jquery('#wenzhangName').val() != null && jquery('#wenzhangName').val() != "" && jquery('#wenzhangName').val() != "null" ) {
                param['wenzhangName'] = jquery('#wenzhangName').val();
            }
                                         
            var wenzhangTypes = $("#wenzhangTypes").find("option:selected").val();
            if (wenzhangTypes != null && wenzhangTypes != "" && wenzhangTypes != "null") {
                            param['wenzhangTypes'] = wenzhangTypes;
                        }
                                                                                                                                            
            // 当前表
                                                
            //收藏只能查类型为1的
            param['wenzhangCollectionTypes'] = 1;

            // 获取列表数据
            http.request('wenzhangCollection/page', 'get', param, function(res) {
